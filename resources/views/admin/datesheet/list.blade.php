        ...
        <div class="box">
            <div class="box-header">
              ...
              <div class="panel-group">
                <div class="panel panel-default">
                  ...
                  <div id="homework_form" class="panel-collapse collapse {{ (@$datesheet || $errors->first())?'in':'' }}">
                    <div class="panel-body ">
                   {{ Form::open(['route'=>(@$datesheet)?['admin.datesheet.update',$datesheet->id]:'admin.datesheet.post','files'=>true]) }}
                             <div class="row">
                                        ...
                                        @foreach($centers as $center)
                                            <div class="col-lg-{{ $n }}">
                                                <div class="radio-custom radio-danger">
                                                    {!! Form::radio('center', $center->id, @$datesheet->center_id, ['required','id'=>'center'.$center->id]) !!}
                                        ...
                                        <div class="col-md-12">
                                             <div class="col-lg-6">                         
                                                <div class="form-group">
                                                    ...
                                                    {!! Form::select('session',$sessions, @$datesheet->session_id, ['class'=>'form-control','placeholder'=>'choose Session','required']) !!}
                                   ...
                                   <div class="col-lg-6">                         
                                      <div class="form-group">
                                          {{ Form::label('title','Title',['class'=>'control-label']) }}
                                           {{ Form::text('title',@$datesheet->title,['class'=>'form-control' ,'required']) }}
                                    ...
                                    <div class="col-lg-6">                         
                                      <div class="form-group">
                                          {{ Form::label('datesheet','Upload file .pdf & .docx',['class'=>'control-label']) }}
                                           {{ Form::file('datesheet',@$datesheet->datesheet,['class'=>'form-control','required']) }}
                                           <p class="text-danger">{{ $errors->first('datesheet') }}
                                           </p>                           
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="col-md-12 text-center">
                            <button class="btn btn-success">{{ (@$datesheet)?'Update':'Upload' }}</button>
                        </div>
                    </div>                            
                    {{ Form::close() }}
                    </div>
            ...
            <div class="box-body">
              <table id="dataTable" class="table table-bordered table-striped table-hover">
                ...
                <tr>
                  <td>{{ $datesheet->id }}</td>
                  <td>{{ $datesheet->created_at }}</td>
                  <td>{{ $datesheet->sessions->date or '' }}</td>
                  <td>{{ $datesheet->classes->name or '' }}</td>
                  <td>{{ $datesheet->title }}</td>

                  
                  
                  <td align="center">
                    <a class="btn btn-info btn-xs" href="{{ route('admin.datesheet.show',$datesheet->id) }}"><i class="fa fa-eye"></i></a>
                    <a class="btn btn-danger btn-xs" onclick="return confirm('Are you sure to delete this data ?')" href="{{ route('admin.datesheet.delete',$datesheet->id) }}"><i class="fa fa-trash"></i></a>
                  </td>
